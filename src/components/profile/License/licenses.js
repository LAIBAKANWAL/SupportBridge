...
import RenderLisence from "./renderLisence";
import { useNavigation } from "@react-navigation/native";
import CustomSwipeListView from "../../../../../compounds/CustomSwipeListView";
import { API_DELETE_LICENSES} from "../../../../../../config/WebService";
import ApiSauce from "../../../../../../services/ApiSauce";


export default function licenses({ data, slug }) {
  ...
  return (
    <View style={{ flex: 1 }}>
      <CustomSwipeListView
           ...
           Alert.alert(
            ...
            [
              ...
              { onDelete(data.item);
               ...
               ApiSauce.delete(API_DELETE_LICENSES +"/"+data.item.id)
                .then((res) => {
                   console.log(res);
        ...
        onEdit={(data) => {
          const { item } = data;
          navigation.navigate("AddLisence", { ...item, slug: slug });
        }}
        renderListItem={(item) => {
          return <RenderLisence item={item} />;
        }}
      ></CustomSwipeListView>
    </View>
  );
