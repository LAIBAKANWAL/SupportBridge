...
import { API_DELETE_CERTIFICATE} from "../../../../../../config/WebService";
import ApiSauce from "../../../../../../services/ApiSauce";


export default function Certificate({ data, slug }) {
  ...
  return (
    <View style={{ flex: 1 }}>
      <CustomSwipeListView
           ...
           Alert.alert(
            ...
            [
              ...
              { onDelete(data.item);
               ...
               ApiSauce.delete(API_DELETE_CERTIFICATE +"/"+data.item.id)
                .then((res) => {
                   console.log(res);
        ...
        onEdit={(data) => {
          const { item } = data;
          navigation.navigate("AddCertificate", { ...item, slug: slug });
        }}
        renderListItem={(item) => {
          return <RenderCertificate item={item} />;
        }}
      ></CustomSwipeListView>
    </View>
  );
