...
const AdminReceiverRequest = () => {
    ...
    const getdata = async () => {
        try {
            const response = await axios.get(`hhttps://app-api.demo-customwebsites.com/api/admin-request-list`);
...
console.log('data',response.data.data)
    ...
    const userObjects = alldata.map(item => item.user);
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.white }}>
            ...
            <View style={{ marginHorizontal: SIZES.medium }}>
                ...
                <Header
                    title="Receiver Request Lists"
                    showBackButton
                />

{userObjects.map((request, index) => (
...
>
    <View style={{ backgroundColor: COLORS.lightGray, borderRadius: 10, alignItems: "center", justifyContent: "center", overflow: 'hidden' }}>
        <Image
            style={{ width: 60, height: 60, resizeMode: 'cover' }}
            source={request.profile_image ? { uri: "https://app-api.demo-customwebsites.com/" + request.profile_image } : require('../../assets/images/images.jpg')}
        />
    ...
    <View style={{ marginLeft: 10, flex: 1 }}>
        ...
        <Text style={styles.notificationMessage}>{request.cnic_number}</Text>
        <Text style={{ color: '#a2a6ab', fontSize: 16, }}>{formatDaysDifference(calculateDaysDifference(request?.created_at))}</Text>

        {(item.is_active === '1' ?
            <Button
                title="Approve"
                onPress={() => navigation.navigate("RequestInfo", { itemId: '2' })}
                filled={false}
                width='40%'
                paddingBottom={5}
                paddingVertical={5}
            ...
            <Button
                title="Reject"
                onPress={() => navigation.navigate("RequestInfo", { itemId: '9' })}
                filled={true}
                width='50%'
                paddingBottom={5}
                paddingVertical={5}
    ...
    <View style={{ flexDirection: 'row' }}>
        <TouchableOpacity onPress={() => navigation.navigate('FundraiserDetails', { itemId: request.id })}>
...
export default AdminReceiverRequest;
